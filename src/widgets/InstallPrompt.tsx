...
export function InstallPrompt({ locale = 'en' }: InstallPromptProps) {
  ...
  useEffect(() => {
    const handler = (e: Event) => {
      ...
      // 保存された表示モードを確認
      const savedMode = localStorage.getItem('csv-viewer-install-prompt-mode') as DisplayMode | null;
      const dismissedTime = localStorage.getItem('csv-viewer-install-prompt-dismissed');

      // 1週間経過していたらリセット
      if (dismissedTime) {
        const oneWeek = 7 * 24 * 60 * 60 * 1000;
        if (Date.now() - parseInt(dismissedTime, 10) >= oneWeek) {
          localStorage.removeItem('csv-viewer-install-prompt-mode');
          localStorage.removeItem('csv-viewer-install-prompt-dismissed');
  ...
  const handleInstall = async () => {
    ...
    setDisplayMode('hidden');
    localStorage.setItem('csv-viewer-install-prompt-mode', 'hidden');
  };

  const handleDismissBanner = () => {
    // バナーを閉じてフッターモードに移行
    setDisplayMode('footer');
    localStorage.setItem('csv-viewer-install-prompt-mode', 'footer');
  };
  ...
  const handleDismissFooter = () => {
    ...
    setDisplayMode('hidden');
    localStorage.setItem('csv-viewer-install-prompt-mode', 'hidden');
    localStorage.setItem('csv-viewer-install-prompt-dismissed', Date.now().toString());
  };
