...
test.describe('covenants', () => {
  ...
  test('links to related tools and its own engineering-notes article (#177)', async ({ page }) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      expect(href, `related card ${i} is not a self-link`).not.toContain('/csv-viewer/');
    ...
    if (await blogLink.count()) {
      await expect(blogLink).toHaveAttribute('href', /^https:\/\/runlocally\.app\/blog\/csv-viewer\/$/);
    }
  });
  ...
  test('opens a file offline after the first online visit (#2)', async ({ page }) => {
    ...
    await page.reload();
    await waitReady(page);
    // Open a file online so the papaparse chunk is fetched through the SW and cached.
    await openSampleCsv(page);
    ...
    try {
      ...
      await waitReady(page);
      await openSampleCsv(page); // view with no network at all
    } finally {
